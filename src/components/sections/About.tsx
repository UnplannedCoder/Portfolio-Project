            ...
            <motion.div variants={itemVariants} className="flex flex-col items-center gap-7">
              ...
              <div className="w-full space-y-2.5">
                {[
                  ...
                  { icon: Github, text: 'UnplannedCoder (GitHub)', href: personalInfo.github },
                  { icon: Linkedin, text: 'pawan-sain-18b74631b', href: personalInfo.linkedin },
            ...
            <motion.div variants={itemVariants} className="space-y-8">
              ...
              <div>
                ...
                {personalInfo.about.split('\n\n').map((para, i) => (
                  <p key={i} className="text-zinc-400 leading-relaxed text-sm mb-3">
                    {para}
                  </p>
                ))}
              </div>
              ...
              <div>
                <h4 className="text-sm font-semibold text-zinc-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                  <GraduationCap className="w-4 h-4 text-violet-400" /> Education
                </h4>
                {education.map((edu) => (
                  <div key={edu.school} className="p-4 rounded-xl bg-zinc-900 border border-zinc-800">
                    <p className="text-white font-semibold text-sm">{edu.school}</p>
                    <p className="text-violet-300 text-sm">{edu.degree} · {edu.major}</p>
                    <p className="text-zinc-500 text-xs mt-1">{edu.duration} · {edu.location}</p>
                  </div>
                ))}
              </div>
              ...
              <div>
                ...
                {experience.map((exp) => (
                  <div key={exp.company} className="p-4 rounded-xl bg-zinc-900 border border-zinc-800">
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <div>
                        <p className="text-white font-semibold text-sm">{exp.role}</p>
                        <p className="text-violet-300 text-sm">{exp.company} · {exp.type}</p>
                      </div>
                      <span className="text-zinc-500 text-xs whitespace-nowrap">{exp.duration}</span>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {exp.tags.map((t) => (
                        <span key={t} className="px-2 py-0.5 rounded-full text-xs bg-violet-500/10 text-violet-300 border border-violet-500/20">{t}</span>
                      ))}
                    </div>
                  </div>
                ))}
